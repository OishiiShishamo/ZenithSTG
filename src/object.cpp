...
namespace zenithstg {
	...
	void Object::MoveObject(double speed) {
		pos_ += vec_ * speed;
	}

	int Object::ColliCheckObject() {
		return 0;
	}

	void Object::GrazeObject() {
	}

	int Object::CheckPosBounds() {
		return 0;
	}

	int Object::CheckCollisionAndBounds() {
		if (flags_ & kIsCol) {
			if (ColliCheckObject()) {
				flags_ &= ~kIsAlive;
				return 1;
