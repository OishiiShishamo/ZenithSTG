...
Player::MovePlayer() {
	...
	if (pos.GetX() < BORDER_LEFT + PLAYER_SIDE_BORDER_OFFSET) pos.SetX(BORDER_LEFT + PLAYER_SIDE_BORDER_OFFSET);
	if (pos.GetX() > BORDER_RIGHT - PLAYER_SIDE_BORDER_OFFSET) pos.SetX(BORDER_RIGHT - PLAYER_SIDE_BORDER_OFFSET);
	if (pos.GetY() > BORDER_DOWN - PLAYER_BOTTOM_BORDER_OFFSET) pos.SetY(BORDER_DOWN - PLAYER_BOTTOM_BORDER_OFFSET);
	if (pos.GetY() < BORDER_UP + PLAYER_TOP_BORDER_OFFSET) pos.SetY(BORDER_UP + PLAYER_TOP_BORDER_OFFSET);
}
...
Player::Shot() {
	if (GetAsyncKeyState(0x5A)) {
		CreatePlayerShot(pos, Color(255, 255, 255), 0, BLEND_ADD, 255, 1, 24, 24, 0, 0, 1.0f, 1.0f, 0, 0, Rad(90), Rad(90), 0, 0, 0, 50, EASEINCUBIC, 60, -1, 0);
	}
}
