...
Particle::UpdateEase() {
	...
	if (pal_ease_time_ == 0) {
		...
		pal_t_ = elapsed_frame / pal_ease_time_;
		if (pal_t_ > 1) pal_t_ = 1;
...
GrazeParticle(const Vec2D& pos) {
	for (int i = 0; i < 1; i++) {
		...
		CreateParticle(pos, zenithstg::Color(kColorWhite), kParticleStar, kBlendAdd, 255, kEaseInQuad, 15, 0, 0, 0, 0, 0, std::fmod(rng() / 100.0f, 1.5f), 0, kEaseInQuad, 120, 0, rand_tmp, rand_tmp, 0, 0, std::fmod(rng() / 100.0f, 32), 0, kEaseInQuad, 120);
	}
}